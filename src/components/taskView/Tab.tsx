...
const Tab = ({ tabName, disabled = false, isColored = false, callback = () => { } }: Props) => {
    ...
    return (
        <li className={"px-2 rounded bg-gray-100 text-gray-600 cursor-pointer select-none"
            + (isColored ? generateColorClasses(colorFromTag(tabName)) : '')
            + (disabled ? '' : ' hover:bg-red-200 hover:text-red-700 hover:line-through')}
